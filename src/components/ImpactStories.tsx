...
import { useEffect } from "react";
// ✅ Local Import
import PrathapImage from "../assets/Prathap_Image.jpg";
        ...
        >
          <motion.div className="flex gap-6 w-max" animate={controls}>
            {loopStories.map((item, index) => (
              <div
                key={index}
                className="group relative w-[320px] min-w-[320px] h-[400px] rounded-2xl overflow-hidden bg-zinc-900 p-8 flex flex-col justify-between transition-all duration-500 shadow-xl"
              >
                {/* 🔹 Background Image: ALWAYS VISIBLE */}
                <img
                  src={item.image}
                  alt={item.name}
                  className="absolute inset-0 w-full h-full object-cover opacity-60 group-hover:opacity-40 transition-opacity duration-500 scale-110 group-hover:scale-100"
                />

                {/* 🔹 Dark Gradient Overlay (helps text readability) */}
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent z-1" />

                {/* 🔹 Top Content: Hidden on load, shows on hover */}
                <div className="relative z-10 opacity-0 group-hover:opacity-100 transition-all duration-500 transform translate-y-4 group-hover:translate-y-0">
                   <img
                    src={item.image}
                    alt={item.name}
                    className="w-12 h-12 rounded-full object-cover border-2 border-white/50 mb-4"
                  />
                  ...
                  {/* 🔹 READ MORE BUTTON */}
                  <button className="mt-4 px-4 py-2 bg-white/10 hover:bg-white text-white hover:text-black text-xs font-bold uppercase tracking-widest rounded-lg border border-white/20 transition-all duration-300 cursor-pointer">
                    Read More
                  </button>
                </div>
